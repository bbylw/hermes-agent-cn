...
>
> **Hermes：** _（写入 `~/.hermes/scripts/memory-watchdog.sh`，然后以 `no_agent=true` 调用 `cronjob(...)`）_
>
...
当你的请求类似"当 X 时告警我"或"每 N 分钟检查 Y，如果 Z 则通知我"，Hermes 的 `cronjob` 工具描述会指示它在消息内容完全由脚本决定时使用 `no_agent=True`。当请求需要推理时（_"总结新 issue"_、_"挑选最有趣的头条"_、_"起草一条友好提醒"_），则回退到普通的 LLM 驱动路径。
...
> **你：** 今晚先停掉内存看门狗
>
> **Hermes：** _（调用 `cronjob(action='pause', job_id='abc123')`）_
>
> 已暂停。说"重新开启"或执行 `hermes cron resume abc123` 可恢复。

> **你：** 改成每 15 分钟一次
>
> **Hermes：** _（调用 `cronjob(action='update', job_id='abc123', schedule='every 15m')`）_
